"""Build ChromaDB fewshot index for DAIL-SQL.
...
data/fewshot/questions.json.

Usage (from project root):
    uv run python src/DAIL-SQL/scripts/build_fewshot_index.py
    uv run python src/DAIL-SQL/scripts/build_fewshot_index.py --rebuild

Options:
    --train_xlsx   Path to training Excel file  (default: data/split_config_II_tiered/train_data.xlsx)
    --data_dir     DAIL-SQL data root           (default: src/DAIL-SQL/data)
    --top_k        Number of similar examples   (default: 3)
...
_DAIL_DATA    = Path(__file__).resolve().parents[1] / "data"
_DEFAULT_XLSX = _PROJECT_ROOT / "data" / "split_config_II_tiered" / "train_data.xlsx"
...
def build_chroma_collection(train_df: pd.DataFrame, chroma_dir: Path,
                             model: SentenceTransformer, rebuild: bool) -> chromadb.Collection:
    ...
    for start in range(0, len(train_df), batch_size):
        ...
        for local_i, (_, row) in enumerate(batch.iterrows()):
            ...
            for variant, sql_col in (("point", "point_query"), ("bbox", "bbox_query")):
                ...
                docs.append(q)
                embs.append(emb)
                metas.append({"sql": str(row.get(sql_col) or ""), "geo_mode": variant,
...
def retrieve_top_k(collection: chromadb.Collection, model: SentenceTransformer,
                   question: str, top_k: int, geo_mode: str) -> list[tuple[str, str]]:
...
def process_eval_file(eval_path: Path, collection: chromadb.Collection,
                      model: SentenceTransformer, top_k: int, by_question_out: dict) -> int:
    ...
    for item in tqdm(items, desc=eval_path.name, unit="q"):
        ...
        by_question_out[raw_q.strip().lower()] = {"prompt": _build_prompt(examples)}
        added += 1
    return added


def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(description="Build ChromaDB fewshot index for DAIL-SQL")
    parser.add_argument("--train_xlsx", default=str(_DEFAULT_XLSX))
    parser.add_argument("--data_dir", default=str(_DAIL_DATA))
    ...
    chroma_dir = data_dir / "chroma_fewshot"
    ...
    output = {"questions": {}, "by_question": by_question_out, "extract": {}}
