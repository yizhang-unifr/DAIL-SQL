"""Meteo semantic hint: LLM-based landcover access-pattern classification.

Mirrors OpenSearch-SQL's landcover_semantic_hints.py exactly — same prompt,
same modes, same rendered hints.  Only landcover questions receive a hint;
all other questions return an empty string.
"""
...
def _classify_mode_llm(question: str, llm_adapter) -> LandcoverSemanticMode:
    if question in _mode_cache:
        return _mode_cache[question]
    prompt = _CLASSIFY_PROMPT.format(question=question)
    try:
        raw = llm_adapter.get_ans(prompt, temperature=0.0)
...
def _is_landcover_question(question: str) -> bool:
    q = question.lower()
    return any(t in q for t in ("landcover_upscaled", "landcover_type")) or any(
        k in q for k in ("landcover", "land cover", "land-cover")
    )


def build_semantic_hint(question: str, llm_adapter=None) -> str:
    """Return a landcover SQL pattern hint, or '' for non-landcover questions."""
    if not _is_landcover_question(question):
        return ""
    if llm_adapter is not None:
        mode = _classify_mode_llm(question, llm_adapter)
    else:
